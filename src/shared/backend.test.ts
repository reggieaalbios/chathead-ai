...
describe('desktop protocol', () => {
  ...
  it('accepts the Rust camel-case local voice snapshot shape', () => {
    expect(backendSnapshotSchema.safeParse({
      providers: [], launchReadiness: { ready: false, blockers: ['missingLaunchProvider', 'desktopIntegrationRequired'] },
      desktopIntegration: { kind: 'gnomeShell', status: 'notInstalled', gnomeVersion: '46' }, overlayRunning: false,
      voice: {
  ...
  it('rejects an out-of-range model progress value', () => {
    expect(backendSnapshotSchema.safeParse({
      providers: [], launchReadiness: { ready: false, blockers: ['missingLaunchProvider'] },
  ...
  it('rejects unknown desktop integration states', () => {
    const integration = backendSnapshotSchema.shape.desktopIntegration
    expect(integration.safeParse({ kind: 'gnomeShell', status: 'ready', gnomeVersion: '46' }).success).toBe(true)
    expect(integration.safeParse({ kind: 'xwayland', status: 'ready' }).success).toBe(false)
  })
})
